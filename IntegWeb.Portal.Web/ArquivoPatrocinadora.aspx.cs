    ...
    public partial class ArquivoPatrocinadora : BasePage
    {
        ...
        private void AutoRefreshDaPagina(int segundos)
        {
            ScriptManager.RegisterClientScriptBlock(upArqPatrocinadoras,
                    upArqPatrocinadoras.GetType(),
                    "script",
                    //String.Format("self.setTimeout('window.location.reload(true)', {0});", segundos * 1000),
                    String.Format("self.setTimeout(\"$('#ContentPlaceHolder1_btnRefresh').click()\", {0});", segundos * 1000),
                    true);
        }
        ...
        //    DownloadDemonstrativo(AnoRef, MesRef, DateTime.Parse(txtDtRepasse2.Text), DateTime.Parse(txtDtCredito2.Text), sCOD_ARQ_PATS);
        //    grdReembolsoAjustes.DataBind();
        //}

        //protected void btnNovoLancamento_Click(object sender, EventArgs e)
        //{
        //    //odsDemonstrativo.Insert();
        //    //grdReembolsoAjustes.ChangeMode(DetailsViewMode.Insert);
        //    //grdReembolsoAjustes.DataBind();
        //}
        ...
        protected void btnCarregar_Click(object sender, EventArgs e)
        {
            ...
            if (lst_carregar.Count > 0)
            {
                ...
                if (res.Ok)
                {
                    string strMensagem = res.Mensagem;
                    ArqPatrocinadoraDAL.PRE_VIEW_ARQ_PATROCINA ap_nota = lst_carregar.FirstOrDefault(ap => ap.TIP_ARQUIVO == 4);
                    ArqPatrocinaNotaDebitoBLL ND = new ArqPatrocinaNotaDebitoBLL();
                    res = ND.ProcessarNotaDebito(ap_nota.ANO_REF ?? 0, ap_nota.MES_REF ?? 0, ap_nota.GRUPO_PORTAL);

                    if (res.Ok)
                    {
                        GerarNotaDebito(0, ap_nota.ANO_REF ?? 0, ap_nota.MES_REF ?? 0, ap_nota.GRUPO_PORTAL);
                        MostraMensagem(TbUpload_Mensagem, strMensagem);
                    }
                    else
                    {
                        MostraMensagem(TbUpload_Mensagem, strMensagem + "Erro ao emitir a Nota de Débito: " + res.Mensagem);
                    }
        ...
        private void GerarNotaDebito(int iCOD_ARQ_PAT, short sANO_REF, short sMES_REF, string pGRUPO_PORTAL)
        {
            ...
            //Resultado res = apBLL.ProcessarNotaDebito(sANO_REF, sMES_REF, pGRUPO_PORTAL, false);
            ...
            if (res.Ok)
            {
                ...
                rel.get_parametro("ANO_REF").valor = sANO_REF.ToString(); // sANO_REF.ToString();
                rel.get_parametro("MES_REF").valor = sMES_REF.ToString(); // sMES_REF.ToString();
                //rel.get_parametro("GRUPO_PORTAL").valor = GRUPO_PORTAL;
                Session[rel.relatorio] = rel;
                AbrirNovaAba(upArqPatrocinadoras, "RelatorioWeb.aspx?Relatorio_nome=" + rel.relatorio + "&PromptParam=false&Popup=true&Alert=false", rel.relatorio);
                //grdArqEnviados.DataBind();
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if ((e.Row.RowState & DataControlRowState.Edit) == 0)                
                {
                    ...
                    if (Array.IndexOf(aStatus, COD_STATUS) > -1)
                    {
                        ...
                        AutoRefreshDaPagina(15);
                    }

                    Label lstCriticas = (Label)e.Row.FindControl("lstAcoes");
                    //Label lstCodEmprs = (Label)e.Row.FindControl("lstCodEmprs");
                    ...
                    //foreach(String cod_emprs in ArqPatBLL.GetCOD_EMPRSs(COD_ARQ_PAT))
                    //{
                    //    lstCodEmprs.Text += cod_emprs + ",";
                    //}
                    //lstCodEmprs.Text = lstCodEmprs.Text.Substring(0, lstCodEmprs.Text.Length - 1);
                }
            }
