...
{
    public partial class IncideJuros : BasePage
    {
        #region Atributos
        IncideJurosBLL objCoe = new IncideJurosBLL();
                ...
                case "Deletar":
                    ...
                    try
                    {
                        if (!hdNumPartif.Value.Equals("0"))
                        {
                            ...
                            objCoe.dt_inic_vig = DateTime.Parse(commandArgs[2]);

                            bool ret = objCoe.DeletarRegistro(out mensagem);

                            if (ret)
                            {
                                objCoe = new IncideJurosBLL();
                                CarregaTela();
                            }
        ...
        protected void btnSalvar_Click(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(txtDtFim.Text))
                    objCoe.dt_fim_vig = DateTime.Parse(txtDtFim.Text);

                if (!string.IsNullOrEmpty(txtDtIni.Text))
                    objCoe.dt_inic_vig = DateTime.Parse(txtDtIni.Text);

                if (!string.IsNullOrEmpty(txtJuros.Text))
                    objCoe.tx_juros = decimal.Parse(txtJuros.Text);
